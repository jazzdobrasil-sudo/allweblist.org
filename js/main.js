/**
 * Main JavaScript for AllWebList.org Directory
...
/**
 * Load all published websites from the database
 */
async function loadWebsites() {
    try {
        ...
        // Filter only published websites
        allWebsites = data.data.filter(site => site.status === 'published');
    ...
    return `
        <div class="bg-white rounded-lg shadow-md p-6 card-hover">
            ...
            <div class="flex justify-between items-center text-xs text-gray-500 pt-4 border-t">
                ...
                <span>
                    <i class="fas fa-user mr-1"></i>${site.submittedBy === 'admin' ? 'Admin' : 'User'}
                </span>
            </div>
        </div>
    `;
